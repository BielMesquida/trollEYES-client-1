var miControlador = miModulo.controller(
    ...
    ['$scope', '$http', '$routeParams', 'auth', '$location',
        function ($scope, $http, $routeParams, auth, $location) {
            ...
            $scope.controller = "tipo_usuarioRemoveController";
            $scope.hecho = false;
            $scope.fallido = false;
            $scope.eliminar = function () {
                $http({
                    method: 'POST',
                    url: 'http://localhost:8081/trolleyes/json?ob=tipo_usuario&op=remove&id='+$routeParams.id
